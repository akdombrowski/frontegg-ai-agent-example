...
export interface IGenParams extends IParams {
	model: string;
	prompt: string;
	suffix?: string;
	images?: string[];
	format?: string;
	// options is model-dependent
	options?: string;
	template?: string;
	raw?: boolean;
	keep_alive?: string;
}
export default function OllamaAgent({ model, prompt }: IGenParams) {
	const HOST = '127.0.0.1:11434';
	const API = '/api';
	const DEFAULT_MODEL_NAME = 'gemma3:latest';

	const generate = async () => {
		const endpoint = `{HOST}{API}/generate`;
		const params: IGenParams = { model: model, prompt: prompt };
		const method = 'POST';
    const res = await fetch(endpoint, { method, body: JSON.stringify(params) });
...
Only use integrations the user has authorized. Be transparent about actions you take.`;
	 ...
	 */
	initializeFronteggAIAgentsClient = async (): Promise<FronteggAiClient> => {
		try {
			const fronteggAiClient = await FronteggAiClient.getInstance({
			...
			return fronteggAiClient;
		} catch (e) {
			const err = e as Error;
			logger.error(`Failed to initialize LLM Agent: ${err.message}`);
			logger.debug(`Stack trace: ${err.stack}`);

			throw new Error('Failed to initialize LLM Agent', { cause: err });
		}
	};
